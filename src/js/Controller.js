...
import { default as highscoreservice } from "./HighscoreServices.js";
...
const handleSubmit = () => {
  if (
    model.getGuessedCode().length !==
    gameDifficulty[model.getDifficulty()].codeLength
  ) {
  ...
  const occurrenceStatus = compareCodes(
    model.getSecretCode(),
    model.getGuessedCode()
  );
...
const startGame = (difficulty) => {
  ...
  initUI();
  eventListener.addEventListenerToPanelButtons(onInput);
};
...
const updateGameLogic = () => {
  if (hasGuessedSecretCode()) {
    ...
    eventListener.addEventListenerToSubmitButton(onSubmitScore);
  } else if (hasLost()) {
...
const hasLost = () => {
  return model.getCurrentTurn() === maxTries;
};
